...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val txnsPath  = args.headOption.getOrElse("data/transactions.csv")
    val prodsPath = args.lift(1).getOrElse("data/products.csv")
    val outDir    = args.lift(2).getOrElse("out")
    val minTotal  = args.lift(3).flatMap(s => Try(s.toDouble).toOption).getOrElse(0.0)
    ...
    try {
      ...
      SparkJobs.runPipeline(
        spark             = spark,
        txnsPath          = txnsPath,
        productsPath      = prodsPath,
        outDir            = outDir,
        minTotalThreshold = minTotal
      )
      println(s"Pipeline finished successfully. Outputs written to '$outDir'.")
    } finally {
      // Cleanup
      spark.stop()
    }
